...
Man definiert eine Operation $\bmod$, sodass $a \bmod b$ der Rest beim Teilen von a durch b ist.\\
...
\begin{beispiel}
Mittels der Rechenregeln von Satz \ref{s:mod} ist es möglich, den Rechenaufwand erheblich zu reduzieren, wenn nicht nach exakten Zahlen, sondern bestimmten Resten gefragt wird. So ist zum Beispiel $25497 + 45038 + 45938 + 1234 \bmod 3 = 0 + 2 + 2 + 1 \bmod 3 = 2$. Die Summe dieser beachtlich großen Zahlen, lässt also beim Teilen durch $3$ den Rest $2$. Dafür haben wir die Zahlen nur kurz angesehen und auch keinen Taschenrechner benötigt.
\end{beispiel}
...
Dazu betrachten wir die Zahl $2017^{2017} \bmod 10$. Das ist ein Produkt von 2017 gleichen Zahlen. Laut Satz \ref{s:mod} ist dann die letzte Ziffer gleich der letzten Ziffer von $7^{2017}$. Weiterhin gilt $7^{2017} \equiv 7^{2 \cdot 1008 + 1} \equiv 7 \cdot (7^2)^{1008} \equiv 7 \cdot 49^{1008} \equiv 7 \cdot (-1)^{1008} \equiv 7 \pmod{10}$
\end{beispiel}


\section{Sätze über Teiler und Faktoren}
...
\begin{beweis}
Den trivialen Fall in der ersten Zeile möchte ich hier direkt überspringen. Betrachten wir den Schritt, welcher in der zweiten Zeile vollzogen wird:\\
...
Induktionsbeweis: Man bestimme Zahlen $q,r$ mit $m = q \cdot n + r$ und $r<n$. Weil $r<n$ gibt es Zahlen $a',b' \in \mathbb{Z}$ mit $ggT(n,r) = a'n + b'r$. Mit Satz \ref{s:eukl} folgt:\\
$ggT(m,n) = ggT(n,r) = a'n + b'r = a'n + b'(m - qn) = b'm + (a'-b'q)n$\\
